<script lang="ts">
  ...
  let kda = $derived(heroStats.avgDeaths > 0 ? ((heroStats.avgKills + heroStats.avgAssists) / heroStats.avgDeaths).toFixed(1) : (heroStats.avgKills + heroStats.avgAssists).toFixed(1))

  let hasMore = $derived(loadedCount < heroStats.matchCount)

  async function fetchChunk(skip: number): Promise<any[]> {
  ...
  function mapMatches(rawMatches: any[]): any[] {
    return rawMatches.map((match: any) => {
      ...
      const outcomeStr = getLaneOutcome({...match, ...playerData})
      let laneResult = null
    ...
    <div class="flex items-center gap-4">
      ...
      <div>
        ...
        <div class="flex items-center gap-3 mt-1 text-sm">
          <div><span class="text-tx3">Matches:</span> <span class="font-bold text-tx2">{heroStats.matchCount}</span></div>
          <div class="w-1 h-1 rounded-full bg-tx3"></div>
          <div><span class="text-tx3">Win Rate:</span> <span class="font-bold text-tx2 {Number(winrate) >= 50 ? 'text-gr' : 'text-rd'}">{winrate}%</span></div>
          <div class="w-1 h-1 rounded-full bg-tx3"></div>
          <div><span class="text-tx3">KDA:</span> <span class="font-bold text-pu2">{kda}</span></div>
        </div>
      </div>
    </div>
  </div>
  ...
  <div class="flex-1 overflow-y-auto overflow-x-hidden p-4 bg-bg">
    ...
    {:else if error && allMatches.length === 0}
      <div class="flex flex-col items-center justify-center gap-3 py-16 text-sm">
        ...
        <button class="sel-pill px-3 py-1 text-sm font-semibold cursor-pointer hover:bg-s3" onclick={fetchInitial}>Retry</button>
      </div>
    {:else if allMatches.length === 0}
      <div class="flex items-center justify-center py-16 text-sm text-tx3">No matches found.</div>
    {:else}
      <div class="flex flex-col gap-[7px] min-w-0" class:opacity-50={loading}>
            ...
            <div class="min-w-0 flex-1 pl-1">
              <div class="flex items-center gap-2">
                <ToolTip text={m.outcome === 'win' ? 'Win' : 'Loss'}>
                  <span
                    class="flex items-center justify-center w-[26px] h-[26px] rounded-full text-sm font-extrabold shrink-0 leading-none {m.outcome === 'win' ? 'text-gr bg-grb' : 'text-rd bg-rdb'}"
                  >
                    {m.outcome === 'win' ? 'W' : 'L'}
                  </span>
                </ToolTip>
                {#if m.laneResult}
                  <ToolTip text={m.laneResult === 'won' ? 'Lane won' : m.laneResult === 'tie' ? 'Lane tied' : 'Lane lost'}>
                    <span
                      class="flex items-center justify-center w-[14px] h-[14px] rounded-[3px] text-[10px] font-extrabold leading-none shrink-0 {m.laneResult === 'won' ? 'text-gr bg-grb' : m.laneResult === 'tie' ? 'text-gd bg-gdb' : 'text-rd bg-rdb'}"
                    >
                ...
                <span class="text-xxs text-tx3 uppercase tracking-wide px-1.5 py-[1px] rounded bg-s2 shrink-0">{m.mode ?? 'Unknown'}</span>
              </div>
            </div>
            ...
            <div class="flex items-center gap-1.5 shrink-0">
              {#if m.items?.length}
                {#each m.items.slice(0, 6) as item}
                  <div class="w-[30px] h-[30px] rounded bg-s2 overflow-hidden border border-bd/40">
