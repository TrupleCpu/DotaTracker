<script lang="ts">
  ...
  const DONUT_COLORS = ['var(--color-pu2)', 'var(--color-gr)', 'var(--color-gd)', 'var(--color-bl)', 'var(--color-rd)']
  ...
  let partyStats = $derived.by(() => {
    const groups: Record<number, { matches: number; wins: number; k: number; d: number; a: number }> = {}
  ...
  let bestHeroes = $derived.by(() => {
    return [...playerStore.allHeroStats]
  ...
  let laneStats = $derived.by(() => {
    const groups: Record<string, { matches: number; wins: number; k: number; d: number; a: number }> = {}
    for (const m of playerStore.detailedMatches) {
...
<div class="flex-1 overflow-y-auto p-4 select-none">
  {#if playerStore.isLoading}
    <div class="flex items-center justify-center py-20">
      <span class="text-sm text-tx3 animate-pulse">Loading analysis…</span>
    </div>
  ...
  {:else}
    <div class="grid lg:grid-cols-3 md:grid-cols-2 grid-cols-1 gap-4 mb-4">
      <div class="card p-4">
        ...
        <div class="text-2xl font-extrabold tabular-nums {winRate >= 55 ? 'text-gr' : winRate >= 48 ? 'text-gd' : 'text-rd'}">{winRate}%</div>
        <div class="text-xs text-tx2 mt-1">{playerStore.playerStats?.matchCount ?? 0} total matches ({playerStore.playerStats?.winCount ?? 0}W / {(playerStore.playerStats?.matchCount ?? 0) - (playerStore.playerStats?.winCount ?? 0)}L)</div>
      </div>
      <div class="card p-4">
        ...
        <div class="text-xs text-tx2 mt-1">{playerStore.playerStats?.gpmAverage ?? 0} GPM &middot; {playerStore.playerStats?.xpmAverage ?? 0} XPM</div>
      </div>
      <div class="card p-4">
        <div class="text-xxs font-bold text-tx3 uppercase tracking-[0.7px] mb-1">Hero Pool</div>
        <div class="text-2xl font-extrabold text-gr tabular-nums">{playerStore.allHeroStats.length}</div>
        <div class="text-xs text-tx2 mt-1">
          {#if roleStats.length > 0}
            Most played: {roleStats[0].label} ({roleStats[0].matches} games)
          {/if}
        </div>
      </div>
    </div>
        ...
        <div class="flex items-start gap-4 pt-1">
          <div class="shrink-0">
            <svg width="130" height="130" viewBox="0 0 130 130">
              <circle cx="65" cy="65" r="52" fill="none" stroke="var(--color-s3)" stroke-width="22" />
              {#each roleDonutSegments as seg (seg.label)}
                {@const r = 52}
                {@const c = 2 * Math.PI * r}
                {@const dash = seg.fraction * c}
                {@const rot = (seg.startAngle - 90)}
                <circle
                  cx="65" cy="65" r={r}
                  fill="none"
                  stroke={seg.color}
                ...
                />
              {/each}
              <text x="65" y="56" text-anchor="middle" fill="var(--color-tx)" font-size="18" font-weight="800" dominant-baseline="middle">
                {roleStats.reduce((s, r) => s + r.matches, 0)}
              </text>
              <text x="65" y="79" text-anchor="middle" fill="var(--color-tx3)" font-size="10" font-weight="600" dominant-baseline="middle">
                games
              </text>
            </svg>
          </div>
          <div class="flex-1 min-w-0 flex flex-col gap-[9px]">
            {#each roleStats as r}
              {@const color = r.winrate >= 55 ? 'var(--color-gr)' : r.winrate >= 48 ? 'var(--color-gd)' : 'var(--color-rd)'}
              <div>
                ...
                <div class="flex items-center gap-2">
                  <div class="flex-1 h-[6px] bg-s3 rounded-sm overflow-hidden">
                    <div class="h-full rounded-sm" style="background: {color}; width: {r.winrate}%"></div>
                  </div>
                  <span class="text-xxs text-tx3 w-10 text-right tabular-nums">{r.matches}</span>
                </div>
              </div>
            {/each}
          </div>
              ...
              <div class="flex items-center gap-3 py-1.5 border-b border-bd last:border-b-0">
                ...
                <div class="flex-1 h-[6px] bg-s3 rounded-sm overflow-hidden">
                  <div
                    ...
                    style="width: {p.winrate}%; background: {p.winrate >= 55 ? 'var(--color-gr)' : p.winrate >= 48 ? 'var(--color-gd)' : 'var(--color-rd)'}"
                  ></div>
                </div>
                <span class="text-xs font-bold w-10 text-right tabular-nums {p.winrate >= 55 ? 'text-gr' : p.winrate >= 48 ? 'text-gd' : 'text-rd'}">{p.winrate}%</span>
          ...
          {#each bestHeroes as h (h.heroId)}
            ...
            <div class="flex items-center gap-2.5 py-1 border-b border-bd last:border-b-0">
              <div class="w-8 h-8 rounded bg-s2 border border-bd/40 overflow-hidden shrink-0">
                <img src={getHeroImgUrl(h.img)} alt={h.name} class="w-full h-full object-cover object-top" />
              </div>
              ...
              <span class="text-xs font-bold w-10 text-right tabular-nums" style="color: {color}">{h.winrate}%</span>
    ...
    <div class="card p-4">
      <div class="card-hd"><span class="card-ttl">Lane Performance <span class="text-tx3 font-normal text-xxs">(last {playerStore.detailedMatches.length} matches)</span></span></div>
      {#if laneStats.length === 0}
        <div class="text-xs text-tx3 py-4 text-center">No match data available</div>
      {:else}
        <div class="flex flex-col gap-2 pt-1">
          {#each laneStats as l}
            {@const color = l.winrate >= 55 ? 'var(--color-gr)' : l.winrate >= 48 ? 'var(--color-gd)' : 'var(--color-rd)'}
            <div class="flex items-center gap-3 py-1.5 border-b border-bd last:border-b-0">
              ...
              <div class="flex-1 h-[6px] bg-s3 rounded-sm overflow-hidden">
                <div class="h-full rounded-sm" style="background: {color}; width: {l.winrate}%"></div>
              </div>
              <span class="text-xs font-bold w-10 text-right tabular-nums" style="color: {color}">{l.winrate}%</span>
